...
class ListOfCourses extends StatefulWidget {
  ...
  const ListOfCourses({Key key, this.form}) : super(key: key);
...
class _ListOfCoursesState extends State<ListOfCourses> {
  ...
  Widget build(BuildContext context) {
    ...
    final courses = DatabaseService('').getAllCoursesByForm(widget.form);
    return Scaffold(
      body: FutureBuilder<List<DocumentSnapshot>>(
          future: courses,
          builder: (context, c) {
            final List<DocumentSnapshot> cc = c.data;
            final items = <Row>[];
            // ignore: prefer_function_declarations_over_variables
            final widget = (int i) {
              return Padding(
                      ...
                      Positioned(
                        child: RaisedButton(
                          padding: EdgeInsets.only(
                              left: getWidth(3.24), right: getWidth(3.24)),
                          color: colors[i % 6],
                          onPressed: () {
                            ...
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => ListOfUnits(
                                        courseID: cc[i].documentID,
                                        courseInfo: cc[i].data,
                                        courseForm: this.widget.form)));
                          },
                          shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(19.0)),
                          child: Align(
